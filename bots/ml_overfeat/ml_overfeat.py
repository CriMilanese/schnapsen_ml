...
DEFAULT_MODEL = os.path.dirname(os.path.realpath(__file__)) + '/model_ml_overfeat.pkl'
...
def get_num_trump_cards(trump_suit, hand):
    ...
    for card in hand:
        ...
        if (index == 3 and trump_suit == "S") or (index == 2 and trump_suit == "H") or (index == 1 and trump_suit == "D") or (index == 0 and trump_suit == "C"):
            num_trump_cards += 1
...
def features(state):
    ...
    # Add opponent's played card rank to feature set
    if opponents_played_card:
        opponents_played_card_rank = opponents_played_card % 5
    else:
        opponents_played_card_rank = 0

    # Add the number of trump cards in player's hand
    num_trump_cards = get_num_trump_cards(trump_suit, state.hand())

    feature_set.append(num_trump_cards)
    feature_set.append(opponents_played_card_rank)
    ...
    # Append who has won and with how many points
    feature_set.append(game_points)
