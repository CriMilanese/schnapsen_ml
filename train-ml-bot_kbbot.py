...
def create_dataset(path, player=kbbot.Bot(), games=2000, phase=1):
...
if options.create_dataset or not os.path.isfile(options.path):
    create_dataset(options.path, player=kbbot.Bot(), games=10000)
...
if options.train:
    ...
    joblib.dump(model, './bots/ml/model_kbbot.pkl')
